...
public class Ctm2csv  {
	...
	public static void main(String [] args) throws Throwable {
		...
		if(args.length>0){
			...
			folder= new File(args[0]+"//transcripts//LIUM_new//");
		}
		else{
			...
			if(files.length!=0)
				for(int index=0;index<files.length;++index){
					...
					if(files[index].toString().endsWith(".ctm")){
						ctmFiles++;
						String inputName=file.getName().split(".ctm")[0].substring(1);
					...
					try {
						...
						bw = new BufferedWriter(new FileWriter(output_folder+"//"+inputName+".transcript-lium_new.csv"));
